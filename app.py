...
def preprocess_image(image):
    # U2Net标准输入尺寸：320x320（如果你的模型是其他尺寸，修改这里）
...
def infer_mask(image, session):
    ...
    output_name = session.get_outputs()[0].name  # U2Net取第一个输出层d0
...
# -------------------------- Streamlit网页界面 --------------------------
st.set_page_config(page_title="U2Net人像分割", page_icon="🎨", layout="wide")
st.title("🎨 U2Net 人像分割工具（预训练ONNX版）")
